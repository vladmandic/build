...
var setFsWatchFileListener = (path5, fullPath, options3, handlers) => {
  ...
  if (cont) {
    ...
    cont = {
      ...
      rawEmitters: rawEmitter,
      options: options3,
      watcher: watchFile(fullPath, options3, (curr, prev) => {
...
var isReadable = (s) => !!s && typeof s === "object" && s instanceof EventEmitter3 && typeof s.pipe === "function" && // node core Writable streams have a pipe() method, but it throws
s.pipe !== Stream.Writable.prototype.pipe;
var isWritable = (s) => !!s && typeof s === "object" && s instanceof EventEmitter3 && typeof s.write === "function" && typeof s.end === "function";
var EOF = Symbol("EOF");
...
import { parse, resolve as resolve3 } from "path";
...
var readdir4 = (path5) => new Promise((res, rej) => fs5.readdir(path5, { withFileTypes: true }, (er, data9) => er ? rej(er) : res(data9)));
...
var stat4 = (path5) => new Promise((res, rej) => fs5.stat(path5, (er, data9) => er ? rej(er) : res(data9)));
var lstat4 = (path5) => new Promise((res, rej) => fs5.lstat(path5, (er, data9) => er ? rej(er) : res(data9)));
...
import { parse as parse2, resolve as resolve4 } from "path";
...
var rimrafPosixDir = async (path5, opt, ent) => {
  ...
  const removedAll = (await Promise.all(entries.map((ent2) => rimrafPosixDir(resolve4(path5, ent2.name), opt, ent2)))).reduce((a, b) => a && b, true);
  if (!removedAll) {
    return false;
  }
...
import { parse as parse5, resolve as resolve7 } from "path";
...
import { basename as basename3, parse as parse4, resolve as resolve6 } from "path";
...
var win32DefaultTmp = async (path5) => {
  ...
  const driveTmp = resolve5(root, "/temp");
...
var uniqueFilename = (path5) => `.${basename3(path5)}.${Math.random()}`;
...
var rimrafMoveRemoveDir = async (path5, opt, ent) => {
  ...
  const removedAll = (await Promise.all(entries.map((ent2) => rimrafMoveRemoveDir(resolve6(path5, ent2.name), opt, ent2)))).reduce((a, b) => a && b, true);
  if (!removedAll) {
    return false;
  }
...
var tmpUnlink = async (path5, tmp, rm3) => {
  const tmpFile = resolve6(tmp, uniqueFilename(path5));
...
var rimrafMoveRemoveDirSync = (path5, opt, ent) => {
  ...
  let removedAll = true;
  for (const ent2 of entries) {
    const p = resolve6(path5, ent2.name);
...
var rimrafWindowsDir = async (path5, opt, ent, state9 = START) => {
  ...
  const removedAll = (await Promise.all(entries.map((ent2) => rimrafWindowsDir(resolve7(path5, ent2.name), opt, ent2, s)))).reduce((a, b) => a && b, true);
  if (state9 === START) {
...
var rimrafWindowsDirSync = (path5, opt, ent, state9 = START) => {
  ...
  for (const ent2 of entries) {
    const s = state9 === START ? CHILD : state9;
    const p = resolve7(path5, ent2.name);
