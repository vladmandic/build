  ...
  handle(url).then((result) => {
    ...
    if (!result || !result.ok || !result.stat) {
      ...
      if ((_b = result == null ? void 0 : result.stat) == null ? void 0 : _b.isFile()) {
        ...
        const stream = !rangeRequest ? fs3.createReadStream(input) : fs3.createReadStream(input, { start: rangeStart, end: rangeEnd });
...
import fs5 from "fs";
import { chmodSync, mkdirSync, readdirSync, renameSync, rmdirSync, rmSync, statSync, unlinkSync } from "fs";
var chmod = (path4, mode) => new Promise((res, rej) => fs5.chmod(path4, mode, (er, ...d) => er ? rej(er) : res(...d)));
var mkdir = (path4, options3) => new Promise((res, rej) => fs5.mkdir(path4, options3, (er, made) => er ? rej(er) : res(made)));
var readdir = (path4) => new Promise((res, rej) => fs5.readdir(path4, (er, data9) => er ? rej(er) : res(data9)));
var rename = (oldPath, newPath) => new Promise((res, rej) => fs5.rename(oldPath, newPath, (er, ...d) => er ? rej(er) : res(...d)));
var rm = (path4, options3) => new Promise((res, rej) => fs5.rm(path4, options3, (er, ...d) => er ? rej(er) : res(...d)));
var rmdir = (path4) => new Promise((res, rej) => fs5.rmdir(path4, (er, ...d) => er ? rej(er) : res(...d)));
var stat = (path4) => new Promise((res, rej) => fs5.stat(path4, (er, data9) => er ? rej(er) : res(data9)));
var unlink = (path4) => new Promise((res, rej) => fs5.unlink(path4, (er, ...d) => er ? rej(er) : res(...d)));
var promises = {
  chmod,
  mkdir,
  readdir,
...
var rimrafPosix = async (path4, opt) => {
  ...
  await Promise.all(entries.map((entry) => rimrafPosix(resolve2(path4, entry), opt)));
...
var rimrafPosixSync = (path4, opt) => {
  ...
  if (!Array.isArray(entries)) {
    ...
    return ignoreENOENTSync(() => unlinkSync(path4));
  }
  for (const entry of entries) {
    rimrafPosixSync(resolve2(path4, entry), opt);
  }
  if (opt.preserveRoot === false && path4 === parse2(path4).root) {
    return;
  }
  return ignoreENOENTSync(() => rmdirSync(path4));
};
...
var rimrafMoveRemove = async (path4, opt) => {
  ...
  await Promise.all(entries.map((entry) => rimrafMoveRemove(resolve4(path4, entry), opt)));
...
var rimrafMoveRemoveSync = (path4, opt) => {
  ...
  for (const entry of entries) {
    rimrafMoveRemoveSync(resolve4(path4, entry), opt);
  }
  if (opt.preserveRoot === false && path4 === parse4(path4).root) {
    return;
  }
  return ignoreENOENTSync(() => tmpUnlinkSync(path4, tmp, rmdirSync));
};
...
var rimrafWindowsDirSync = retryBusySync(fixEPERMSync(rmdirSync));
...
var rimrafWindows = async (path4, opt, state9 = START) => {
  ...
  await Promise.all(entries.map((entry) => rimrafWindows(resolve5(path4, entry), opt, state9 === START ? CHILD : state9)));
  if (state9 === START) {
...
var rimrafWindowsSync = (path4, opt, state9 = START) => {
  ...
  for (const entry of entries) {
    ...
    rimrafWindowsSync(resolve5(path4, entry), opt, s);
  }
  if (state9 === START) {
...
var rimrafNativeSync = (path4, opt) => rmSync(path4, {
  ...opt,
  force: true,
  recursive: true
});
