...
async function run(config, entry) {
  ...
  const td = new Application();
  td.options.addReader(new TSConfigReader());
  td.options.addReader(new TypeDocReader());
  ...
  if (config.generate) {
    if (existsSync("typedoc.json"))
      ...
      writeFileSync("typedoc.json", JSON.stringify(td.options._values, null, 2));
  ...
  const generated = existsSync(join(entry.typedoc, "index.html"));
  if (result)
...
async function run2(config, entry) {
  ...
  compilerOptions["include"] = [dirname(entry.input)];
  ...
  const program = createProgram([entry.input], compilerOptions.options, compilerHost);
  if (config.generate) {
    if (existsSync2("tsconfig.json"))
      ...
      writeFileSync2("tsconfig.json", JSON.stringify(tsconfig, null, 2));
      log3.info("Generate config file:", ["tsconfig.json"]);
    }
  }
  const emit = program.emit();
  const diag = getPreEmitDiagnostics(program).concat(emit.diagnostics);
...
function handle(url) {
  ...
  return new Promise((resolve) => {
    if (checkFile(join2(process.cwd(), options.documentRoot, url)))
      ...
      resolve(result);
    else if (checkFile(join2(process.cwd(), options.documentRoot, options.defaultFolder, url)))
      resolve(result);
    else if (checkFile(join2(process.cwd(), options.documentRoot, options.defaultFolder, url, options.defaultFile)))
      resolve(result);
    else if (checkFolder(join2(process.cwd(), options.documentRoot, url)))
      resolve(result);
    else if (checkFolder(join2(process.cwd(), options.documentRoot, options.defaultFolder, url)))
      resolve(result);
    else if (checkFolder(join2(dirname2(join2(process.cwd(), options.documentRoot, options.defaultFolder, url, options.defaultFile, url)))))
      resolve(result);
    else
      resolve(result);
  });
  ...
  handle(url).then((result) => {
    ...
    if (!result || !result.ok || !result.stat) {
      ...
      if ((_b = result == null ? void 0 : result.stat) == null ? void 0 : _b.isFile()) {
        const ext = String(extname(input)).toLowerCase();
        ...
        const compress = createBrotliCompress({ params: { [constants.BROTLI_PARAM_QUALITY]: 5 } });
        const stream = fs3.createReadStream(input);
        if (!accept)
          stream.pipe(res);
        else
      ...
      if ((_c = result == null ? void 0 : result.stat) == null ? void 0 : _c.isDirectory()) {
        ...
        dir = dir.map((f) => join2(decodeURI(req.url), f));
...
async function start2(config) {
  ...
  if (fs3.existsSync(options.sslKey) && fs3.existsSync(options.sslCrt)) {
    ...
    try {
      ...
      options.sslKey = join2(dirname2(home), "..", options.sslKey);
      options.sslCrt = join2(dirname2(home), "..", options.sslCrt);
...
async function run4(config) {
  const json = existsSync3(".eslintrc.json") ? JSON.parse(readFileSync(".eslintrc.json").toString()) : {};
  const options3 = {
    ...json,
  ...
  if (config.generate) {
    if (existsSync3(".eslintrc.json"))
      log7.warn("Generate config file exists:", [".eslintrc.json"]);
    else {
      writeFileSync3(".eslintrc.json", JSON.stringify(options3, null, 2));
